...
log_event() {
    ...
    printf '{"ts": %s, "action": "%s", "path": "%s", "detail": "%s"}\n' \
        "$ts" "$action" "${path_//\"/\\\"}" "${detail//\"/\\\"}" >> "$EVENTS_FILE"
}
...
compact_manifest_unlocked() {
    ...
    if ! awk -F '\t' '
        ...
        FILENAME == ARGV[2] {
            ...
            if ($1 == "COMMIT" && tx != "" && $2 == tx) {
                for (i = 1; i <= pending_count; i++) {
                    ...
                    if (fields[1] == "DELETE") {
                        # Keep a tombstone. Without it, a reinstalled/offline
                        # client cannot distinguish "never existed" from
                        # "deleted later" and can resurrect stale data.
        ...
        END {
            ...
            for (path in state) print state[path]
        }
...
cmd_journal_append() {
    ...
    if (( journal_size > JOURNAL_MAX_BYTES )); then
        compact_manifest_unlocked || log ERROR "compattazione journal fallita"
    fi
    flock -u 9
    exec 9>&-
